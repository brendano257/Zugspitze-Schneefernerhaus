...
from ebas.domain.basic_domain_logic.time_period import (
    estimate_period_code, estimate_resolution_code, estimate_sample_duration_code
)

from processing.constants import EBAS_REPORTING_COMPOUNDS, PROPOSED_AUTOMATIC_DETECTION_LIMITS
...
def get_data():
    ...
    for compound, (dates, mrs, flags) in final_joined_data.items():
        for date, mr, flag in zip(dates, mrs, flags):
            ...
            date = date.replace(tzinfo=timezone(timedelta(hours=1))).astimezone(timezone.utc).replace(tzinfo=None)
...
def set_fileglobal_metadata(nas):
    ...
    nas.metadata.revdate = datetime(2021, 6, 1, 0, 0, 0)
    nas.metadata.revision = '1'
    nas.metadata.revdesc = ('Generated by Brendan Blanchard using data finalization procedures specified in '
...
def set_variables(nas, serial_data):
    ...
    for compound in EBAS_REPORTING_COMPOUNDS:
        ...
        if compound in PROPOSED_AUTOMATIC_DETECTION_LIMITS:
            # detection limits are 3x the uncertainty (calculated elsewhere)
            metadata.detection_limit = [
                round(PROPOSED_AUTOMATIC_DETECTION_LIMITS[compound], 3),
                'pmol/mol'
            ]

            # re-calc uncertainty as 1/3 the detection limit
            metadata.uncertainty = [
                round(PROPOSED_AUTOMATIC_DETECTION_LIMITS[compound] / 3, 3),
                'pmol/mol'
            ]
